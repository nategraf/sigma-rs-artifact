...
impl TestHarness {
    ...
    pub fn open_invite(
        ...
        let open_inv = OpenInvite(*invite);
        let encoded: Vec<u8> = bincode::serialize(&(request.clone(), open_inv)).unwrap();
        ...
        let encoded_resp: Vec<u8> = bincode::serialize(&open_invitation_response).unwrap();
        let reply = open_invitation_response.unwrap();
        let resp_len = encoded_resp.len();
        let resp_handle_start = Instant::now();
        let lox_cred = open_invite::handle_response(client_state, reply.0);
    ...
    pub fn trust_promotion(
        ...
        let encoded: Vec<u8> = bincode::serialize(&tp_request).unwrap();
        let req_len = encoded.len();
        ...
        let encoded_resp: Vec<u8> = bincode::serialize(&trust_promo_response).unwrap();
    ...
    pub fn migration(
        ...
        let encoded: Vec<u8> = bincode::serialize(&mig_request).unwrap();
        let req_len = encoded.len();
        let resp_start: Instant = Instant::now();
        ...
        let encoded_resp: Vec<u8> = bincode::serialize(&migration_response).unwrap();
        let resp_len: usize = encoded_resp.len();
    ...
    pub fn level_up(&mut self, rng: &mut (impl CryptoRng + RngCore), cred: Lox) -> (PerfStat, Lox) {
        ...
        let encoded: Vec<u8> = bincode::serialize(&level_up_request).unwrap();
        let req_len = encoded.len();
        let resp_start: Instant = Instant::now();
        ...
        let encoded_resp: Vec<u8> = bincode::serialize(&level_up_response).unwrap();
        let resp_len: usize = encoded_resp.len();
    ...
    pub fn issue_invite(
        ...
        let encoded: Vec<u8> = bincode::serialize(&issue_invite_request).unwrap();
        let req_len = encoded.len();
        let resp_start: Instant = Instant::now();
        ...
        let encoded_resp: Vec<u8> = bincode::serialize(&issue_invite_response).unwrap();
        let resp_len: usize = encoded_resp.len();
    ...
    pub fn redeem_invite(
        ...
        let redeem_invite_request =
            redeem_invite::request(rng, cred, self.ba.lox_pub.clone(), self.ba.today());
        let req_t = req_start.elapsed();
        let (redeem_invite_request, redeem_invite_client_state) = redeem_invite_request.unwrap();
        let encoded: Vec<u8> = bincode::serialize(&redeem_invite_request).unwrap();
        let req_len = encoded.len();
        let resp_start: Instant = Instant::now();
        ...
        let encoded_resp: Vec<u8> = bincode::serialize(&redeem_invite_response).unwrap();
        let resp_len: usize = encoded_resp.len();
        let response = redeem_invite_response.unwrap();
        let resp_handle_start: Instant = Instant::now();
        let i_cred = redeem_invite::handle_response(redeem_invite_client_state, response);
        let resp_handle_t = resp_handle_start.elapsed();
        assert!(i_cred.is_ok(), "Handle response should succeed");
        (
            PerfStat {
    ...
    pub fn check_blockage(
        ...
        let encoded: Vec<u8> = bincode::serialize(&check_blockage_request).unwrap();
        let req_len = encoded.len();
        let resp_start: Instant = Instant::now();
        ...
        let encoded_resp: Vec<u8> = bincode::serialize(&check_blockage_response).unwrap();
        let resp_len: usize = encoded_resp.len();
    ...
    pub fn update_cred(
        ...
        self.ba.rotate_lox_keys(rng);
        let req_start = Instant::now();
        let key = old_key.clone();
        let update_cred_request = update_cred::request(rng, cred, self.ba.lox_pub.clone());
        let req_t = req_start.elapsed();
        assert!(
        ...
        let encoded: Vec<u8> = bincode::serialize(&update_request).unwrap();
        let req_len = encoded.len();
        let resp_start: Instant = Instant::now();
        let update_response = self.ba.handle_update_cred(key, update_request);
        let resp_t: Duration = resp_start.elapsed();
        assert!(
            update_response.is_ok(),
            "Update cred response from server should succeed"
        );
        let encoded_resp: Vec<u8> = bincode::serialize(&update_response).unwrap();
        let resp_len: usize = encoded_resp.len();
        let response = update_response.unwrap();
        let resp_handle_start: Instant = Instant::now();
        let new_cred = update_cred::handle_response(update_state, response);
        let resp_handle_t = resp_handle_start.elapsed();
        assert!(new_cred.is_ok(), "Handle response should succeed");
        (
            PerfStat {
    ...
    pub fn update_invite(
        ...
        self.ba.rotate_invitation_keys(rng);
        let key = old_key.clone();
        let req_start = Instant::now();
        let update_invite_request =
            update_invite::request(rng, cred, self.ba.invitation_pub.clone());
        let req_t = req_start.elapsed();
        assert!(
        ...
        let req_len = encoded.len();
        let resp_start: Instant = Instant::now();
        let update_response = self.ba.handle_update_invite(key, update_request);
        let resp_t: Duration = resp_start.elapsed();
        assert!(
            update_response.is_ok(),
        ...
        let response = update_response.unwrap();
        let resp_handle_start: Instant = Instant::now();
        let new_cred = update_invite::handle_response(update_state, response);
        let resp_handle_t = resp_handle_start.elapsed();
        assert!(new_cred.is_ok(), "Handle response should succeed");
        (
            PerfStat {
