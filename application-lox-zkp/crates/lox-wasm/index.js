import init, {
  ...
  handle_blockage_migration,
    update_invite, handle_update_invite, update_cred, handle_update_cred,
  set_panic_hook, get_last_upgrade_time, get_trust_level, get_invites_remaining, get_next_unlock, get_received_invite_expiry, get_bridge_fingerprint, get_bridgelines_from_bucket} from "./pkg/lox_wasm.js";
...
let requested_trust_promo = await init().then(() => {
    ...
    let req = requested_days("/advancedays", num.toString()).then((today)=> {
        console.log("Advanced days by "+num+" days to: "+ today.toString());
...
let requested_level_two = await level_up(lox_cred, encrypted_table, pubkeys, server_today);
...
let requested_level_three = await level_up(lox_cred, encrypted_table, pubkeys, server_today);
...
console.log("Got pubkeys: " + pubkeys);
let reqstate_update_invitation = update_invite(prepared_invitation, pubkeys, new_pubkeys);

console.log("Success: " + reqstate_update_invitation);
// Update Invitation cred
let new_prepared_invitation = await init().then(() => {
set_panic_hook();
let cred = requested_update_invite("/updateinvite", reqstate_update_invitation).then((response)=> {
  console.log("Got updated Invite: " + response);
  return handle_update_invite(reqstate_update_invitation, response, new_pubkeys);
 });
  return cred;
});
...
await simple_request("/rotate_lox_keys");
 ...
 let reqstate_update_cred = update_cred(lox_cred, new_pubkeys, new_new_pubkeys);

 // Update Invitation cred
 let new_prepared_cred = await init().then(() => {
   set_panic_hook();
   let cred = requested_update_cred("/updatecred", reqstate_update_cred).then((response)=> {
     console.log("Got updated cred: " + response);
     return handle_update_cred(reqstate_update_cred, response, new_new_pubkeys);
   });
   return cred;
   });

let bridgelines = get_bridgelines_from_bucket(lox_cred, encrypted_table);
...
let requested_blockage_migration = blockage_migration(new_prepared_cred, check_migration_cred, new_new_pubkeys);
  ...
  lox_cred = await init().then(() => {
    ...
    let cred = requested_cred("/blockagemigration", requested_blockage_migration).then((response)=> {
      ...
      return handle_blockage_migration(requested_blockage_migration, response, new_new_pubkeys);
    });
    return cred;
    });
