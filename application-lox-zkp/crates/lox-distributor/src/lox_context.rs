...
impl LoxServerContext {
    ...
    pub fn rotate_lox_keys(&self) -> Response<BoxBody<Bytes, Infallible>> {
        let mut ba_obj = self.ba.lock().unwrap();
        ba_obj.rotate_lox_keys(); // FOR TESTING ONLY
    ...
    pub fn rotate_invite_keys(&self) -> Response<BoxBody<Bytes, Infallible>> {
        let mut ba_obj = self.ba.lock().unwrap();
        ba_obj.rotate_invitation_keys(); // FOR TESTING ONLY
    ...
    fn update_cred(&self, req: update_cred::Request) -> Result<update_cred::Response, ProofError> {
        self.ba.lock().unwrap().handle_update_cred(req)
    }

    fn update_invite(
        &self,
        req: update_invite::Request,
    ) -> Result<update_invite::Response, ProofError> {
        self.ba.lock().unwrap().handle_update_invite(req)
    }
    ...
    pub fn verify_and_send_update_cred(
        ...
        let req: update_cred::Request = match serde_json::from_slice(&request) {
            Ok(req) => req,
            Err(e) => {
                let response = json!({"error": e.to_string()});
                let val = serde_json::to_string(&response).unwrap();
        ...
        match self.update_cred(req) {
            ...
            Err(e) => {
                println!("Invalid Update Credential request, Proof Error");
                let response = json!({"error": e.to_string()});
                let val = serde_json::to_string(&response).unwrap();
                prepare_header(val)
            }
    ...
    pub(crate) fn verify_and_send_update_invite(
        ...
        let req: update_invite::Request = match serde_json::from_slice(&request) {
            Ok(req) => req,
            Err(e) => {
                let response = json!({"error": e.to_string()});
                let val = serde_json::to_string(&response).unwrap();
        ...
        match self.update_invite(req) {
            ...
            Err(e) => {
                println!("Invalid Update Invite request, Proof Error");
                let response = json!({"error": e.to_string()});
                let val = serde_json::to_string(&response).unwrap();
                prepare_header(val)
            }
